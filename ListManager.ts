...
export class ListManager {
  ...
  static createUserSelectList(myUser) {
    ...
    for (let i = 0; i < DataStorage.applicants.length; i++) {
      ...
      option.text =
        DataStorage.applicants[i].ApplicantName +
        ' ' +
        DataStorage.applicants[i].ApplicantID;
  ...
  static createCitySelectList(myCitySelectBox, citiesName?) {
    ...
    const myCityOpportunitySelect: HTMLInputElement = document.getElementById(
      'myCityOpportunitySelect'
    ) as HTMLInputElement;
    var selectedCitiesName =
      myCityOpportunitySelect.options[myCityOpportunitySelect.selectedIndex]
        .value;
    const citiesList = DataStorage.cities.filter(
      item => item.CityName == citiesName
    );
  ...
  static updateCityOpportunitySelectionList() {
    let myCityOpportunitySelectBox = document.getElementById(
      'cityOpportunityField'
    );
  ...
  static createUserList(userList) {
    ...
    for (let i = 0; i < userList.length; i++) {
      ...
      cellText = document.createTextNode(
        userList[i].ApplicantTypeBasedOnEducation
      );
      cell.appendChild(cellText);
        ...
        if (confirmDelete) {
          DataStorage.applicants = DataStorage.applicants.filter(
            applicant =>
              ...
              userList[i].ApplicantName + userList[i].ApplicantID
          );
  ...
  static createCityOpportunityList(cityOpportunityList) {
    let cityOpportunityTable = document.getElementById(
      'cityOpportunityTableId'
    );
    ...
    for (let i = 0; i < cityOpportunityList.length; i++) {
      ...
      cell = document.createElement('td');
      cellText = document.createTextNode(
        cityOpportunityList[i].OpportunityName
      );
      cell.appendChild(cellText);
      row.appendChild(cell);
      ...
      cell = document.createElement('td');
      cellText = document.createTextNode(
        cityOpportunityList[i].TopLimitYearValue
      );
      cell.appendChild(cellText);
      ...
      cellButtonDelete.addEventListener('click', function() {
        let confirmDelete = confirm(
          'Şehir - Olanak bilgisini silmek istediğinize emin misiniz? \n Dikkat, Bu işlem geri alınamaz!'
        );
        if (confirmDelete) {
          DataStorage.cities = DataStorage.cities.filter(
            cityOpportunity =>
              cityOpportunity.CityName + cityOpportunity.OpportunityName !=
              cityOpportunityList[i].CityName +
                cityOpportunityList[i].OpportunityName
          );
  ...
  static createCardList(cardList) {
    ...
    for (let i = 0; i < cardList.length; i++) {
      ...
      cellButtonDelete.addEventListener('click', function() {
        let confirmDelete = confirm(
          'Kart bilgisini silmek istediğinize emin misiniz? \n Dikkat, Bu işlem geri alınamaz!'
        );
        if (confirmDelete) {
          DataStorage.cards = DataStorage.cards.filter(
            card => card._identity != cardList[i]._identity
          );
      ...
      cellEditButton.innerHTML = 'Düzenle';
      cellEditButton.addEventListener('click', function() {
        const opportunityArea: HTMLElement = document.getElementById(
          'cityField'
        );
        opportunityArea.style.display = 'inline';
        window.location.href = '#cardPage';
        TableManager.editCardTable(
          cardList,
          i,
          myOpportunityAmount,
          mySelectedOpportunity
        );
        return;
      });
      cell.appendChild(cellEditButton);
